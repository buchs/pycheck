...
fpo2 = open('follow-up.py','w')
fpo2.write("""#! /usr/bin/python
import csv
import sys
import re # regular expressions
import MySQLdb
...
#     DateEnter Date not null);
  ...
  fpo.write('\\N\t%d\t%s\t%s\t%d\t%s\t%d\t%s\t%s\n' % \
        (r[0],r[2],amount,r[3],r[8],acct,r[1],today))

  # Check if it was reconciled, and if so, mark it in our Reconciled table
  if r[4] == "R":
    pystatement = """sql=\"\"\"select Id from Transactions where Type=%d AND Payee="%s" AND Amount=%s AND Category=%d AND Memo="%s" AND Account=%d AND DateTrans="%s" AND DateEnter="%s" \"\"\"""" \
                  % (r[0],r[2],amount,r[3],r[8],acct,r[1],today)
    fpo2.write(pystatement)
    fpo2.write("""
cursor.execute(sql)
id=cursor.fetchone()
